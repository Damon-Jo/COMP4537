...
class PokemonNoToken extends PokemonBadRequest {
  constructor(message) {
    ...
    this.message = "Error - Invalid Token : There is no valid token";
    this.pokeErrCode = 400;
  }
}
...
class PokemonNotAdmin extends PokemonBadRequest {
  constructor(message) {
    ...
    this.message = "Error - Improper user type : Only admin can access this route";
    this.pokeErrCode = 400;
  }
}

class PokemonNotFoundUser extends PokemonBadRequest {
  constructor(message) {
    super(message);
    this.name = "PokemonNotFoundUser";
    this.message = "Error - User was not found: check your requestNot a user";
    this.pokeErrCode = 400;
  }
}

class PokemonWrongPassword extends PokemonBadRequest {
  constructor(message) {
    super(message);
    this.name = "PokemonWrongPassword";
    this.message = "Error - Improper password : Password is wrong";
    this.pokeErrCode = 400;
  }
}
